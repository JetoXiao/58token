...
func TestForwardAsChatCompletions_UpstreamRequestErrorReturnsFailover(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{err: errors.New("tls verify failed")}
...
func TestForwardAsChatCompletions_RawUpstreamRequestErrorReturnsFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestForwardAsChatCompletions_StreamEmitsUsageChunkByDefault(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_chat_usage_default"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	require.False(t, result.UsageIncomplete)
	require.Contains(t, rec.Body.String(), `"usage":{"prompt_tokens":11,"completion_tokens":5,"total_tokens":16`)
	require.Contains(t, rec.Body.String(), `data: [DONE]`)
}

func TestForwardAsChatCompletions_TerminalWithoutUsageMarksIncomplete(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstreamBody := strings.Join([]string{
		...
		`data: {"type":"response.output_text.delta","delta":"ok"}`,
		"",
		`data: {"type":"response.completed","response":{"id":"resp_1","object":"response","model":"gpt-5.4","status":"completed","output":[{"type":"message","id":"msg_1","role":"assistant","status":"completed","content":[{"type":"output_text","text":"ok"}]}]}}`,
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_chat_usage_missing"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	require.True(t, result.UsageIncomplete)
