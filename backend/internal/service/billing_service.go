...
type ModelPricing struct {
	...
	CacheCreationPriceExplicit         bool    // 是否由渠道/区间定价显式设定
...
func (s *BillingService) initFallbackPricing() {
	...
	// OpenAI GPT-5.5 official pricing.
...
func (s *BillingService) applyModelSpecificPricingPolicy(model string, pricing *ModelPricing) *ModelPricing {
	...
	needsGPT56LongContextReset := isGPT56 && (pricing.LongContextInputThreshold > 0 ||
		pricing.LongContextInputMultiplier > 0 || pricing.LongContextOutputMultiplier > 0)
	if !needsLongContextPolicy && !needsCacheCreationPolicy && !needsGPT56LongContextReset {
		return pricing
	}
	cloned := *pricing
	if isGPT56 {
