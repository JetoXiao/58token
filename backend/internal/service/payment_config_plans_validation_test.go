...
func TestValidatePlanRequired_AllValid(t *testing.T) {
	err := validatePlanRequired("Pro", 1, 9.99, 30, "days", nil, nil, nil)
	require.NoError(t, err)
}

func TestValidatePlanRequired_EmptyName(t *testing.T) {
	err := validatePlanRequired("", 1, 9.99, 30, "days", nil, nil, nil)
	require.Error(t, err)
...
func TestValidatePlanRequired_ZeroGroupID(t *testing.T) {
	err := validatePlanRequired("Pro", 0, 9.99, 30, "days", nil, nil, nil)
	require.Error(t, err)
...
func TestValidatePlanRequired_NameValidatedFirst(t *testing.T) {
	err := validatePlanRequired("", 0, 0, 0, "", nil, nil, nil)
	require.Error(t, err)
	require.Contains(t, err.Error(), "plan name")
}
...
func TestValidatePlanRequired_NegativeOriginalPrice(t *testing.T) {
	neg := -10.0
	err := validatePlanRequired("Pro", 1, 9.99, 30, "days", &neg, nil, nil)
	require.Error(t, err)
...
func TestValidatePlanRequired_ZeroOriginalPrice(t *testing.T) {
	zero := 0.0
	err := validatePlanRequired("Pro", 1, 9.99, 30, "days", &zero, nil, nil)
	require.NoError(t, err)
}

func TestValidatePlanRequired_ValidOriginalPrice(t *testing.T) {
	op := 19.99
	err := validatePlanRequired("Pro", 1, 9.99, 30, "days", &op, nil, nil)
	require.NoError(t, err)
}

func TestValidatePlanRequired_ValidLimitedOffer(t *testing.T) {
	offer := 99.0
	expiresAt := time.Now().Add(72 * time.Hour)
	err := validatePlanRequired("Pro", 1, 129, 30, "days", nil, &offer, &expiresAt)
	require.NoError(t, err)
}

func TestValidatePlanRequired_LimitedOfferNeedsExpiry(t *testing.T) {
	offer := 99.0
	err := validatePlanRequired("Pro", 1, 129, 30, "days", nil, &offer, nil)
	require.Error(t, err)
	require.Contains(t, err.Error(), "limited offer")
}

func TestValidatePlanRequired_LimitedOfferMustBeLowerThanPrice(t *testing.T) {
	offer := 129.0
	expiresAt := time.Now().Add(72 * time.Hour)
	err := validatePlanRequired("Pro", 1, 129, 30, "days", nil, &offer, &expiresAt)
	require.Error(t, err)
