...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	cacheReq := responseCacheRequest(c, "/v1/responses", "openai_responses", reqModel, reqStream, body, apiKey)
	...
	for {
		...
		mergeOpsSchedulerDecision(c, scheduleDecision)
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if service.IsOpenAIStreamTransportFailure(err) {
					...
					h.gatewayService.ClearOpenAIStickySession(c.Request.Context(), apiKey.GroupID, sessionHash)
					reqLog.Warn("openai.stream_transport_interrupted",
						zap.Int64("account_id", account.ID),
						zap.Error(err),
					)
				}
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	cacheReq := responseCacheRequest(c, "/v1/messages", "openai_anthropic_messages", reqModel, reqStream, body, apiKey)
	cacheDecision, cacheInflightOwner, cacheServed := tryResponseCacheBeforeForward(c, h.responseCache, cacheReq)
	if cacheServed {
		return
	}
	...
	for {
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					runtimeBlocked := h.gatewayService.RecordOpenAIAccountFailoverForModel(c.Request.Context(), account, currentRoutingModel, failoverErr)
					// 池模式：同账号重试
					if !runtimeBlocked && failoverErr.RetryableOnSameAccount {
						retryLimit := account.GetPoolModeRetryCount()
						if failoverErr.MaxSameAccountRetries > 0 && failoverErr.MaxSameAccountRetries < retryLimit {
				...
				h.gatewayService.ReportOpenAIAccountScheduleResultForModel(account.ID, currentRoutingModel, false, nil)
				if service.IsOpenAIStreamTransportFailure(err) {
					h.gatewayService.RecordOpenAIAccountStreamTransportFailureForModel(c.Request.Context(), account, currentRoutingModel)
					h.gatewayService.ClearOpenAIStickySession(c.Request.Context(), apiKey.GroupID, sessionHash)
					reqLog.Warn("openai_messages.stream_transport_interrupted",
						zap.Int64("account_id", account.ID),
						zap.Error(err),
					)
				}
		...
		if result != nil {
			h.gatewayService.ReportOpenAIAccountScheduleResultForModel(account.ID, currentRoutingModel, true, result.FirstTokenMs)
		} else {
			h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, true, nil)
		}
		finishResponseCacheAfterForward(c, h.responseCache, cacheDecision, cacheEntry, cacheCaptureReason, cacheInflightOwner)
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	setOpsRequestParamsFromBody(c, firstMessage)
