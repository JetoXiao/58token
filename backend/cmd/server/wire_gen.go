...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	freeQuotaRepository := repository.NewFreeQuotaRepository(client, db)
	freeQuotaService := service.NewFreeQuotaService(freeQuotaRepository, settingService)
	billingCacheService := service.ProvideBillingCacheService(billingCache, userRepository, userSubscriptionRepository, apiKeyRepository, userRPMCache, userGroupRateRepository, configConfig, freeQuotaService)
	apiKeyCache := repository.NewAPIKeyCache(redisClient)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, freeQuotaService)
	...
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, emailQueueService, promoService, subscriptionService, affiliateService, freeQuotaService)
	...
	usageLogRepository := repository.NewUsageLogRepository(client, db, secretEncryptor)
	usageService := service.NewUsageService(usageLogRepository, userRepository, client, apiKeyAuthCacheInvalidator)
	usageHandler := handler.NewUsageHandler(usageService, apiKeyService)
	redeemHandler := handler.NewRedeemHandler(redeemService)
	freeQuotaHandler := handler.NewFreeQuotaHandler(freeQuotaService)
	...
	adminUserHandler := handler.ProvideAdminUserHandler(adminService, concurrencyService, affiliateService)
	...
	runtimeModelProbeService := service.NewRuntimeModelProbeService(accountRepository, accountTestService, rateLimitService, time.Minute)
	runtimeModelProbeService.Start()
	openAIGatewayService.SetRuntimeModelProbeService(runtimeModelProbeService)
	...
	opsRepository := repository.NewOpsRepository(db, secretEncryptor)
	...
	runtimeModelProbeService.SetGatewayServices(gatewayService, openAIGatewayService)
	gatewayService.SetRuntimeModelProbeService(runtimeModelProbeService)
	...
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, notificationEmailService, freeQuotaService)
	...
	opsHandler := handler.ProvideAdminOpsHandler(opsService, configConfig)
	...
	trialCardHandler := admin.NewTrialCardHandler(freeQuotaService)
	helpCenterService := service.NewHelpCenterService(settingRepository)
	helpCenterHandler := handler.ProvideAdminHelpCenterHandler(helpCenterService, configConfig)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, trialCardHandler, helpCenterHandler)
	...
	modelPricingHandler := handler.NewModelPricingHandler()
	modelMarketplaceRepository := repository.NewModelMarketplaceRepository(db)
	modelMarketplaceService := service.NewModelMarketplaceService(modelMarketplaceRepository)
	modelMarketplaceHandler := handler.NewModelMarketplaceHandler(modelMarketplaceService)
	handlerHelpCenterHandler := handler.ProvideHelpCenterHandler(helpCenterService, userService, configConfig)
	visitorAnalyticsService := service.NewVisitorAnalyticsService(db)
	visitorAnalyticsHandler := handler.NewVisitorAnalyticsHandler(visitorAnalyticsService)
	downloadResourceObjectStoreFactory := repository.NewS3DownloadResourceStoreFactory()
	downloadResourceService := service.NewDownloadResourceService(db, settingRepository, downloadResourceObjectStoreFactory, redisClient, secretEncryptor)
	downloadResourcesHandler := handler.NewDownloadResourcesHandler(downloadResourceService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, freeQuotaHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPricingHandler, modelMarketplaceHandler, handlerHelpCenterHandler, visitorAnalyticsHandler, downloadResourcesHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, schedulerSnapshotService, tokenRefreshService, accountExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, openAIGatewayService, runtimeModelProbeService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner)
