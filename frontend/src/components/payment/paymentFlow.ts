...
export type VisiblePaymentMethod = 'alipay' | 'wxpay' | 'stripe' | 'airwallex' | 'usdt' | 'balance'
...
export function buildCreateOrderPayload(input: BuildCreateOrderPayloadInput): CreateOrderRequest {
  ...
  const targetUserEmail = (input.targetUserEmail || '').trim()
  if (targetUserEmail) {
    payload.target_user_email = targetUserEmail
  }
  if (normalizedOrigin) {
