<template>
  <div class="space-y-4">
    ...
    <template v-else-if="outcome === 'expired'">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          <div class="flex h-16 w-16 items-center justify-center rounded-full bg-accent-100 dark:bg-accent-900/30">
...
const qrBorderClass = computed(() => {
  if (isAlipay.value) return 'border-primary-400 bg-primary-50 dark:border-primary-400/70 dark:bg-primary-950/20'
  if (isWxpay.value) return 'border-accent-400 bg-accent-50 dark:border-accent-400/70 dark:bg-accent-950/20'
  return 'border-gray-200 bg-white dark:border-dark-600 dark:bg-dark-800'
})

const qrLogoBgClass = computed(() => {
  if (isAlipay.value) return 'bg-primary-500'
  if (isWxpay.value) return 'bg-accent-500'
  return 'bg-gray-400'
})
