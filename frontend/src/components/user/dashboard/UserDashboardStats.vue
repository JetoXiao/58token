<template>
  ...
  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
    <!-- Balance -->
    <div v-if="!isSimple" class="card group overflow-hidden p-5 card-hover">
      <div class="flex items-center gap-3">
        <div class="rounded-2xl bg-primary-100 p-3 ring-1 ring-primary-500/10 dark:bg-primary-400/10">
        ...
        <div>
          ...
          <p class="text-2xl font-semibold tracking-tight text-primary-600 dark:text-primary-300">${{ formatBalance(displayBalance ?? balance) }}</p>
    ...
    <!-- API Keys -->
    <div class="card group overflow-hidden p-5 card-hover">
      <div class="flex items-center gap-3">
        <div class="rounded-2xl bg-cyan-100 p-3 ring-1 ring-cyan-500/10 dark:bg-cyan-400/10">
          <Icon name="key" size="md" class="text-cyan-600 dark:text-cyan-300" :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.apiKeys') }}</p>
          <p class="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ stats?.total_api_keys || 0 }}</p>
          <p class="text-xs text-primary-600 dark:text-primary-300">{{ stats?.active_api_keys || 0 }} {{ t('common.active') }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card group overflow-hidden p-5 card-hover">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ stats?.today_requests || 0 }}</p>
    ...
    <div class="card group overflow-hidden p-5 card-hover">
      <div class="flex items-center gap-3">
        <div class="rounded-2xl bg-accent-100 p-3 ring-1 ring-accent-500/10 dark:bg-accent-400/10">
          <Icon name="dollar" size="md" class="text-accent-600 dark:text-accent-300" :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.todayCost') }}</p>
          <p class="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">
            <span class="text-accent-600 dark:text-accent-300" :title="t('dashboard.actual')">${{ formatCost(stats?.today_actual_cost || 0) }}</span>
          ...
          <p class="text-xs">
            ...
            <span class="text-accent-600 dark:text-accent-300" :title="t('dashboard.accountingCost')">${{ formatCost(totalAccountingCost) }}</span>
  ...
  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
    ...
    <div class="card p-5 card-hover">
      <div class="flex items-center gap-3">
        <div class="rounded-2xl bg-accent-100 p-3 ring-1 ring-accent-500/10 dark:bg-accent-400/10">
          <Icon name="cube" size="md" class="text-accent-600 dark:text-accent-300" :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.todayTokens') }}</p>
          <p class="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ formatTokens(stats?.today_tokens || 0) }}</p>
    ...
    <div class="card p-5 card-hover">
      <div class="flex items-center gap-3">
        <div class="rounded-2xl bg-indigo-100 p-3 ring-1 ring-indigo-500/10 dark:bg-indigo-400/10">
          <Icon name="database" size="md" class="text-indigo-600 dark:text-indigo-400" :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.totalTokens') }}</p>
          <p class="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ formatTokens(stats?.total_tokens || 0) }}</p>
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
          ...
          <div class="flex items-baseline gap-2">
            <p class="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ formatTokens(stats?.rpm || 0) }}</p>
    ...
    <div class="card p-5 card-hover">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ formatDuration(stats?.average_duration_ms || 0) }}</p>
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <span class="font-mono text-sm text-accent-600 dark:text-accent-300" :title="t('dashboard.actual')">
