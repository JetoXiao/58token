          ...
          <div v-if="statusInfo" class="fade-up flex items-center justify-center mb-2">
            <div class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-gray-200 bg-white/90 shadow-sm backdrop-blur-sm dark:border-dark-700 dark:bg-dark-900/90">
              <span
                ...
                :class="statusInfo.isActive ? 'bg-primary-500' : 'bg-accent-500'"
              ></span>
...
function getUsageColor(pct: number): string {
  if (pct > 90) return 'text-accent-500'
...
const detailRows = computed<DetailRow[]>(() => {
  ...
  if (data.mode === 'quota_limited') {
    if (data.quota) {
      const remainColor = data.quota.remaining <= 0 ? 'text-accent-500'
        : data.quota.remaining < data.quota.limit * 0.1 ? 'text-primary-500'
        : 'text-cyan-500'
      rows.push({
    ...
    if (data.expires_at) {
      ...
      rows.push({
        iconBg: 'bg-primary-500/10', iconColor: 'text-primary-500', iconSvg: ICON_CALENDAR,
    ...
    const remainColor = data.remaining != null
      ? (data.remaining <= 0 ? 'text-accent-500' : data.remaining < 10 ? 'text-primary-500' : 'text-cyan-500')
      : ''
    rows.push({
      iconBg: 'bg-primary-500/10', iconColor: 'text-primary-500', iconSvg: ICON_SHIELD,
