  ...
  <AppLayout>
    <div class="mx-auto max-w-7xl space-y-6">
      ...
      <template v-else-if="stats">
        <UserDashboardStats :stats="stats" :balance="user?.balance || 0" :display-balance="dashboardBalance" :is-simple="authStore.isSimpleMode" />
...
import { ref, computed, onMounted } from 'vue'; import { useAuthStore, useFreeQuotaStore } from '@/stores'; import { usageAPI, type UserDashboardStats as UserStatsType } from '@/api/usage'
import AppLayout from '@/components/layout/AppLayout.vue'; import LoadingSpinner from '@/components/common/LoadingSpinner.vue'
import UserDashboardStats from '@/components/user/dashboard/UserDashboardStats.vue'; import UserDashboardCharts from '@/components/user/dashboard/UserDashboardCharts.vue'
import UserDashboardRecentUsage from '@/components/user/dashboard/UserDashboardRecentUsage.vue'; import UserDashboardQuickActions from '@/components/user/dashboard/UserDashboardQuickActions.vue'
import type { UsageLog, TrendDataPoint, ModelStat } from '@/types'

const authStore = useAuthStore(); const freeQuotaStore = useFreeQuotaStore(); const user = computed(() => authStore.user)
const stats = ref<UserStatsType | null>(null); const loading = ref(false); const loadingUsage = ref(false); const loadingCharts = ref(false)
const trendData = ref<TrendDataPoint[]>([]); const modelStats = ref<ModelStat[]>([]); const recentUsage = ref<UsageLog[]>([])
const dashboardBalance = computed(() => freeQuotaStore.summary.total_amount || user.value?.balance || 0)
...
const loadStats = async () => { loading.value = true; try { await authStore.refreshUser(); if (authStore.user?.balance != null) { freeQuotaStore.setBalanceAmount(authStore.user.balance) }; await freeQuotaStore.fetchSummary(true).catch((error) => { console.error('Failed to fetch free quota summary:', error) }); stats.value = await usageAPI.getDashboardStats() } catch (error) { console.error('Failed to load dashboard stats:', error) } finally { loading.value = false } }
